...
public class Labyrinthe {
    ...
    public Labyrinthe(Cellule[][] labCel) {
    	this.hauteur = labCel.length;
    	if (hauteur == 0) {
    		throw new IllegalArgumentException("Hauteur de 0 invalide.");
    	}
    	this.labyrintheCellules = labCel;
    	this.largeur = labCel[0].length;
    	this.verifIntegriteLabyrinthe();
    }
    ...
    /**
     * Vérifie si les murs extérieurs de la cellule aux coordonnées spécifiées sont correctement fermés.
     * Si un mur extérieur est ouvert (false) à un endroit où il devrait être fermé (true), une exception IllegalArgumentException est levée.
     * 
     * @param x         La coordonnée X de la cellule.
     * @param y         La coordonnée Y de la cellule.
     * @param murNord   L'état du mur nord de la cellule.
     * @param murEst    L'état du mur est de la cellule.
     * @param murSud    L'état du mur sud de la cellule.
     * @param murOuest  L'état du mur ouest de la cellule.
     * @throws IllegalArgumentException Si un mur extérieur est ouvert à un endroit où il devrait être fermé.
     */
    private void verifierMursExterieurs(int x, int y, boolean murNord, boolean murEst, boolean murSud,
        ...
        if (x == hauteur - 1 && !murSud) {
            throw new IllegalArgumentException("Les murs extérieurs doivent être fermés.");
        }
        if (y == 0 && !murOuest) {
            throw new IllegalArgumentException("Les murs extérieurs doivent être fermés.");
        }
        if (y == largeur - 1 && !murEst) {
            throw new IllegalArgumentException("Les murs extérieurs doivent être fermés.");
        }
    }
    ...
    private void verifierContraintesAjoutCellule(int x, int y, boolean murNord, boolean murSud, boolean murEst,
        ...
        if (y > 0 && this.labyrintheCellules[x][y - 1] != null
                && this.labyrintheCellules[x][y - 1].estMurPresent('E') != murOuest) {
            throw new IllegalArgumentException("Le mur Ouest de la cellule (" + x + ", " + y
                    + ") ne correspond pas au mur Est de la cellule (" + x + ", " + (y-1) + ").");
        }

        if (y < largeur - 1 && this.labyrintheCellules[x][y + 1] != null
                && this.labyrintheCellules[x][y + 1].estMurPresent('O') != murEst) {
            throw new IllegalArgumentException("Le mur Est de la cellule (" + x + ", " + y
                    + ") ne correspond pas au mur Ouest de la cellule (" + x + ", " + (y+1) + ").");
        }

        if (x > 0 && this.labyrintheCellules[x - 1][y] != null
                && this.labyrintheCellules[x - 1][y].estMurPresent('S') != murNord) {
            throw new IllegalArgumentException("Le mur Nord de la cellule (" + x + ", " + y
                    + ") ne correspond pas au mur Sud de la cellule (" + (x-1) + ", " + y + ").");
        }

        if (x < hauteur - 1 && this.labyrintheCellules[x + 1][y] != null
                && this.labyrintheCellules[x + 1][y].estMurPresent('N') != murSud) {
            throw new IllegalArgumentException("Le mur Sud de la cellule (" + x + ", " + y
                    + ") ne correspond pas au mur Nord de la cellule (" + (x+1) + ", " + y + ").");
        }
    ...
    public void verifIntegriteLabyrinthe() {
    	...
    	for (int i = 0; i < this.hauteur; i++) {
    		for (int j = 0; j < this.largeur; j++) {
    			Cellule cell = this.getCellule(i, j);
