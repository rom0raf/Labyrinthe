...
public class CreationLabyrinthe {
   ...
   public static Labyrinthe creationAscendante(int hauteur, int largeur){
        ...
        int yPremiereCellule = (int)(Math.random() * (tableauLaby.length + 1));
        int xPremiereCellule = (int)(Math.random() * (tableauLaby[0].length + 1));
        ...
        // Récupération de la deuxième cellule aléatoire
        boolean[] directionsPossibles = {true, true, true, true}; // N, S, E, O
      
        if (numeroPremiereCellule < tableauLaby[0].length) {
     	    directionsPossibles[0] = false;
        }
        if (numeroPremiereCellule >= tableauLaby.length * (tableauLaby[0].length - 1)) {
       	    directionsPossibles[1] = false;
        }
        if (numeroPremiereCellule % tableauLaby[0].length == tableauLaby[0].length - 1)  {
       	    directionsPossibles[2] = false;
        }
        if (numeroPremiereCellule % tableauLaby[0].length == 0)  {
        ...
        int direction;
        do {
         	direction = (int)(Math.random() * (directionsPossibles.length + 1));
